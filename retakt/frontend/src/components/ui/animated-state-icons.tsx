...
interface StateIconProps {
  ...
  color?: string;
  className?: string;
  /** Controlled state — true = second state (success/pause/unlocked/etc) */
  active?: boolean;
}
...
/* ─── 3. PLAY → PAUSE ─── */
export function PlayPauseIcon({ size = 40, color = "currentColor", className, active = false }: StateIconProps) {
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
  ...
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      ...
      <motion.circle cx="20" cy="26" r="2" fill={color}
        animate={active ? { scale: 0.6, opacity: 0.4 } : { scale: 1, opacity: 1 }}
...
/* ─── 5. COPY → COPIED ─── */
export function CopiedIcon({ size = 40, color = "currentColor", className, active = false }: StateIconProps) {
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
...
/* ─── 6. BELL → NOTIFICATION ─── */
export function NotificationIcon({ size = 40, color = "currentColor", className, active = false }: StateIconProps) {
  return (
...
/* ─── 7. HEART → FILLED ─── */
export function HeartIcon({ size = 40, color = "currentColor", className, active = false }: StateIconProps) {
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      <motion.path
        d="M20 34s-12-7.5-12-16a7.5 7.5 0 0112-6 7.5 7.5 0 0112 6c0 8.5-12 16-12 16z"
        stroke={active ? "#EF4444" : color} strokeWidth={2} fill={active ? "#EF4444" : "none"}
...
/* ─── 9. VOLUME ─── mute/unmute */
export function VolumeIcon({ size = 40, color = "currentColor", className, active = false }: StateIconProps) {
  return (
    <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
      ...
      <motion.path d="M26 14a8 8 0 010 12" stroke={color} strokeWidth={2} strokeLinecap="round"
        animate={active ? { opacity: 0, x: -3 } : { opacity: 1, x: 0 }}
        transition={{ duration: 0.3 }}
      />
      <motion.path d="M30 10a14 14 0 010 20" stroke={color} strokeWidth={2} strokeLinecap="round"
        animate={active ? { opacity: 0, x: -5 } : { opacity: 0.5, x: 0 }}
        transition={{ duration: 0.3, delay: 0.05 }}
      />
      <motion.g animate={active ? { opacity: 1 } : { opacity: 0 }} transition={{ duration: 0.25 }}>
