-- Fix: allow public (anon) to read comments so non-logged-in visitors can see them
-- Also ensure the table exists in case migration v0_1_3 was never applied
...
-- Drop old policies (both old name and new name) to avoid conflicts
...
-- Public read: anyone (logged in or not) can read comments
