...
export function CanvasText({
  ...
  const updateColors = useCallback(() => {
    if (bgRef.current) {
      ...
      const newBgColor = computed.backgroundColor;
      // Ensure we're getting a valid color, fallback to a default if needed
      if (newBgColor && newBgColor !== 'rgba(0, 0, 0, 0)' && newBgColor !== 'transparent') {
        setBgColor(prev => prev === newBgColor ? prev : newBgColor);
      }
    }
